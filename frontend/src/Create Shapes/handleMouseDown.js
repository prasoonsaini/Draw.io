...
const handleMouseDown = (canvasRef, e, setIsDrawing, setCurrentShape, shape, setShape, offsetX, offsetY, zoomLevel) => {
    ...
    setIsDrawing(true)
    if (shape === 'rec') {
        setCurrentShape({ shape: 'rec', x: mouseX, y: mouseY, width: 0, height: 0, current: true, strokeColor: '#e03131', backgroundColor: 'white', fillType: 'solid', strokeWidth: 2, slopiness: 0, strokeStyle: [0, 0] });
    }
    else if (shape === 'cir') {
